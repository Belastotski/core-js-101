...
function isCreditCardNumber(ccn) {
  let arr = [...`${ccn}`].reverse();
  arr = arr.map((x, i) => {
    if (i % 2) return 2 * x > 9 ? 2 * x - 9 : 2 * x;
    return x;
  });
  return arr.reduce((sum, x) => +sum + +x) % 10 === 0;
}
...
function getDigitalRoot(num) {
  if (num < 9) return num;
  return getDigitalRoot([...`${num}`].reduce((s, x) => +s + +x));
}
